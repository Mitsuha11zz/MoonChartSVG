...
### 2. SVG 弧线计算 — 三角函数替代
饼图需要 sin/cos 计算弧线端点。采用 Taylor 级数近似（4 阶展开）配合角度归约算法（归一化到 [0, π/2] 范围），精度满足 SVG 渲染需求，坐标不会超出画布范围。
...
- 78 个测试全部通过（`moon test --deny-warn`）
- 测试类型：白盒测试 + 黑盒测试
- 覆盖范围：每种图表类型的基本渲染、多系列、空数据边界情况、格式化输出、配置选项、数据统计函数
